...
- The file must `#include` the `scm.h` header. When `gcc` is called, this header will be part of the includes search path, so there's no need to worry about where it resides.

- All functions that are to be exported by the C module must accept and return values of type `scm_value_t`. This type is defined in `scm.h`, and there are functions declared in the header to manipulate and work with these values.
...
scm_native_export_t garlic_c_exports[] = {
    {"add", add, 2},
    0
};
```
